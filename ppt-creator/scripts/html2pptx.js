...
async function htmlToPptx(htmlFilePath, outputPptxPath, options = {}) {
    ...
    // 提取 head 中的 CDN 资源（script、link 标签），保留 Tailwind 等动态样式所需的资源
    const headResources = extractHeadResources(htmlContent);
    ...
    try {
        ...
        for (let i = 0; i < slidePages.length; i++) {
            ...
            // 创建完整的 HTML 文档（单个幻灯片）
            const fullHtml = wrapSlideHtml(fixedSlideHtml, styles, headResources);
...
/**
 * 从 HTML 内容中提取 head 中的 CDN 资源标签（script、link）
 * 仅在提取不到时才回退到搜索整个 HTML
 */
function extractHeadResources(htmlContent) {
    const headMatch = htmlContent.match(/<head[^>]*>([\s\S]*?)<\/head>/i);
    if (!headMatch) return '';
    
    // 先移除 HTML 注释，避免提取注释内的标签
 ...
 */
function wrapSlideHtml(slideHtml, originalStyles = '', headResources = '') {
